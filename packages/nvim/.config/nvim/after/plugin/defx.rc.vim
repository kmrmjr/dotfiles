...
nnoremap <silent>fi :<C-u>Defx `expand('%:p:h')` -search=`expand('%:p')`<CR>

" Start Defx when Vim starts with a directory argument.
autocmd StdinReadPre * let s:std_in=1
autocmd VimEnter * if argc() == 1 && isdirectory(argv()[0]) && !exists('s:std_in') |
    \ execute 'Defx -show-ignored-files -columns=indent:mark:icon:space:icons:space:filename:git:size' argv()[0] | endif
